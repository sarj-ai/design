...
```
src/app/(mockups)/<slug>/page.tsx        one route per mockup — the group is not in the URL
src/components/mockups/<slug>/           that mockup's components
src/components/mockups/<slug>/icons.tsx  its icons, named for what they do here
...
src/lib/site/reels-data.ts               the reel index registry
src/lib/site/thumbnails.json             card preview fingerprints — generated by npm run thumbs
...
.claude/skills/                          every skill, one folder each — see below
docs/audits/                             audits of the product against the system
docs/handoffs/                           handoff notes for one piece of work
...
`src/lib/site/mockups-data.ts` — are tracked and pushed. Only build output,
review screenshots, and env files stay out of git.
...
**Every file has a group.** Code for one mockup goes in that mockup's
`mockups/<slug>` folders. The chrome screens sit in goes in `shell/`, a building
block more than one area uses goes in `shared/`, and the landing index's own code
goes in `site/`. The design system and reels keep their own folders. Nothing new
goes at the top of `src/components` or `src/lib` — the one loose file,
`src/lib/utils.ts`, is shadcn's, and the primitives import it from exactly there.
Reports and notes about the work go in `docs/`, never the repo root.
...
**Icons.** Each mockup owns `src/components/mockups/<slug>/icons.tsx`, built on the
`icon()` factory in `src/components/shared/icon.tsx`, naming each glyph for what it does
