 ...
 *
 * Filed under `src/components/ui` for the same reason as the other vendored
 * visual pieces: it is authored against arbitrary `em` values, a gradient
 * gloss, a ring and its own z-stack, none of which the house lint rules allow
 * in authored code. This directory is where those rules are off.
 *
 * One change from the source: the glow was a fixed cyan. It is now
 * `--chart-6`, the bright end of the brand's purple ramp, so the reveal reads
 * as the same family as everything around it and follows a brand change. The
 * `hue-rotate` sweep then travels from that purple rather than from cyan.
 */
...
export default function TextInlineChipReveal({
  ...
  const variants = useMemo(() => {
    const word: Variants = {
      ...
      shown: (i: number) => ({
        opacity: 1,
        transition: { duration: reduced ? 0 : FADE, delay: reduced ? 0 : delay + i * stagger, ease: "linear" },
      }),
    };
    const glow: Variants = {
      ...
      shown: (i: number) => ({
        ...
        transition: reduced
          ? { duration: 0 }
          : {
              opacity: { duration: INK, delay: delay + i * stagger + GLOW_HOLD, ease: "linear" },
              filter: { duration: total, delay, ease: "linear" },
            },
      }),
    };
    const ink: Variants = {
      ...
      shown: (i: number) => ({
        ...
        transition: reduced
          ? { duration: 0 }
          : { duration: INK, delay: delay + i * stagger + GLOW_HOLD, ease: "linear" },
      }),
    };
    return { word, glow, ink };
  }, [reduced, stagger, total, delay]);
            ...
            >
              ...
              >
                ...
                <motion.span
                  ...
                  style={{
                    textShadow:
                      "0 0 0.06em var(--chart-6), 0 0 0.2em var(--chart-6)",
                  }}
