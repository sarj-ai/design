...
description: Design and implement web animations that feel natural and purposeful — easing, timing, duration, springs, enter/exit transitions, container morphs, performance, and reduced motion — based on Emil Kowalski's "Animations on the Web". Use ALWAYS when adding, changing, or reviewing any animation, transition, or motion in this repo: dialogs and drawers opening, panels resizing, content swapping, hover and press feedback, loading and skeleton states, list reordering, or whenever something "jumps", "feels janky", or "should be animated".
---
...
## House rules for this repo (Sarj mockups)

These apply on top of everything above and win where they conflict.

- **Easing tokens live in `src/app/globals.css`.** Use `ease-out-cubic` (entering / exiting) and
  `ease-in-out-cubic` (an on-screen element resizing or moving). Never write a raw
  `cubic-bezier()` in a component.
- **Durations:** hover and press 150ms · tooltips, popovers, dropdowns 200ms · dialog and drawer
  open/close 200–300ms · a container morphing to fit new content 250–300ms. Nothing over 300ms.
- **Reduced motion is not optional.** Every animated element carries `motion-reduce:transition-none`
  or `motion-reduce:animate-none`. No exceptions, including opacity.
- **Enter animations come from `tw-animate-css`,** already imported globally: `animate-in`,
  `fade-in-0`, `slide-in-from-bottom-1`, `zoom-in-95`. Don't hand-write keyframes for these.
- **Never animate a value the design system owns.** No animating a primitive's padding, radius, or
  colour tokens — animate `transform`, `opacity`, and (only for a container fitting new content)
  `height`.
- **Height is the one permitted layout animation,** and only on a wrapper that measures its child
  with a `ResizeObserver`. It is allowed because switching a settings section is a rare interaction,
  not a 100×/day one. Anywhere frequent, don't animate height at all.
- **RTL:** slide directions must be logical. Prefer `slide-in-from-bottom-*` over
  `slide-in-from-left/right-*`, which do not mirror.
