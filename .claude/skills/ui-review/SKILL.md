...
3. **The platform repo** (`sarj-ai/platform`, default branch `dev`) for the current behavior baseline: read the existing page/component the design replaces under `products/platform/apps/web/src/app/` to build the list of existing features and behaviours that must survive the redesign. This powers checklist gate "all existing features kept" with evidence instead of memory.
